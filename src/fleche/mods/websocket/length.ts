...
export class Length {
  ...
  sizeOrThrow() {
    if (this.value < 126) return 7;
    if (this.value < 65_535) return 7 + 16;
    return 7 + 64;
  }
  ...
  writeOrThrow(cursor: Cursor) {
    if (this.value < 126) return this.#writeOrThrow7(cursor);
    if (this.value < 65_535) return this.#writeOrThrow16(cursor);
  ...
  static readOrThrow(cursor: Cursor) {
    ...
    if (header < 126) return new Length(header);
    if (header === 126)
