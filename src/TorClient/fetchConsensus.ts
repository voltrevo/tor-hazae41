...
/**
 * Options for fetching the Tor consensus.
 */
export interface FetchConsensusOptions {
...
/**
 * Fetches the Tor consensus in an isolated way.
 * Creates a temporary Tor connection and circuit, fetches the consensus,
 * and cleans up all resources.
 *
 * This is ideal for one-time consensus fetching where you don't need
 * persistent circuit management.
 *
 * @param options Configuration options for the consensus fetch
 * @returns Promise resolving to the consensus object
 *
 * @example
 * ```typescript
 ...
 * });
 *
 * console.log(`Fetched consensus with ${consensus.microdescs.length} microdescs`);
 * ```
 */
export async function fetchConsensus(options: FetchConsensusOptions) {
  const {
    snowflakeUrl,
