...
export function decodeOnionPubKey(host: string): Uint8Array {
  ...
  const basename = host.slice(0, -'.onion'.length);

  // v3 addresses are exactly 56 base32 chars
  if (!/^[a-z2-7]{56}$/.test(basename)) {
    throw new Error(
      'Invalid v3 onion address format (expect 56 base32 chars).'
    );
  }

  // 2) Base32-decode → 35 bytes (32 pubkey | 2 checksum | 1 version)
  const decoded = Base32.fromString(basename);
  ...
  if (version !== 0x03)
    throw new Error('Unsupported onion address version (expected 0x03).');
