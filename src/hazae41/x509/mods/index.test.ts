...
import { readAndResolveFromBytesOrThrow, writeToBytesOrThrow } from "./types/index";
...
await test("Cert Ed25519", async () => {
  const bytes = PEM.decodeOrThrow(await readFile("./certs/ed25519.pem", "utf8"))
...
await test("Cert Let's Encrypt", async () => {
  const bytes = PEM.decodeOrThrow(await readFile("./certs/letsencrypt.pem", "utf8"))
  const cert = readAndResolveFromBytesOrThrow(Certificate, bytes)
...
test("ISRG Root X1", async () => {
  const bytes = PEM.decodeOrThrow(await readFile("./certs/isrg-root-x1.pem", "utf8"))
  const cert = readAndResolveFromBytesOrThrow(Certificate, bytes)
