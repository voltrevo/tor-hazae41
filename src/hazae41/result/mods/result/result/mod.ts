...
export namespace Result {
   ...
   * @param wrapper
   */
  export function rewrap<T extends Result.Infer<T>>(
    wrapper: T
  ): Result<Ok.Inner<T>, Err.Inner<T>>;
   ...
   * @see Err.throw
   */
  export async function unthrow<R extends Result.Infer<R>>(
    callback: (thrower: (e: Err.Infer<R>) => void) => Awaitable<R>
  ): Promise<R> {
    ...
    try {
      ...
      if (ref !== undefined) return ref as R;
      throw e;
    }
  }
   ...
   * @see Err.throw
   */
  export function unthrowSync<R extends Result.Infer<R>>(
    callback: (thrower: (e: Err.Infer<R>) => void) => R
  ): R {
    let ref: Err.Infer<R> | undefined;
   ...
   * @returns
   */
  export async function runAndWrap<T>(
   ...
   * @returns
   */
  export async function runAndDoubleWrap<T>(
    callback: () => Awaitable<T>
   ...
   * @returns
   */
  export async function runOrWrap<R extends Result.Infer<R>>(
    callback: () => Awaitable<R>
  ): Promise<R | Err<unknown>> {
    try {
   ...
   * @returns
   */
  export function runOrWrapSync<R extends Result.Infer<R>>(
    callback: () => R
  ): R | Err<unknown> {
    try {
      return callback();
   ...
   * @returns
   */
  export async function runOrDoubleWrap<R extends Result.Infer<R>>(
    callback: () => Awaitable<R>
  ): Promise<R | Err<Error>> {
    try {
      return await callback();
   ...
   * @returns
   */
  export function runOrDoubleWrapSync<R extends Result.Infer<R>>(
    callback: () => R
  ): R | Err<Error> {
    try {
      return callback();
  ...
  export function maybeAll<T, E>(
    iterable: Iterable<Nullable<Result<T, E>>>
  ): Nullable<Result<Array<T>, E>> {
  ...
  export function* maybeIterate<T, E>(
    iterable: Iterable<Nullable<Result<T, E>>>
  ): Iterator<T, Nullable<Result<void, E>>> {
    for (const result of iterable) {
  ...
  export function collect<T, E>(
    iterator: Iterator<T, Result<void, E>>
  ): Result<Array<T>, E> {
  ...
  export function maybeCollect<T, E>(
    iterator: Iterator<T, Nullable<Result<void, E>>>
  ): Nullable<Result<Array<T>, E>> {
    const array = new Array<T>();
