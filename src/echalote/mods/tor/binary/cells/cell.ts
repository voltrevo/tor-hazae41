...
export type Cell<T extends Writable> = Cell.Circuitful<T> | Cell.Circuitless<T>;
...
export namespace Cell {
  ...
  export class Raw<T extends Writable> {
    ...
    static readOrThrow(cursor: Cursor) {
      ...
      if (command >= 128) {
        ...
        return new Raw<Unknown>(circuit, command, payload);
      }
  ...
  export class Circuitful<T extends Writable> {
    ...
    static from<T extends Cellable.Circuitful & Writable>(
    ...
    static intoOrThrow<T extends Writable>(
      cell: Cell<Unknown>,
      readable: Cellable.Circuitful & Readable<T>
    ) {
  ...
  export class Circuitless<T extends Writable> {
    ...
    static from<T extends Cellable.Circuitless & Writable>(
      circuit: undefined,
      cellable: T
    ) {
    ...
    static intoOrThrow<T extends Writable>(
      cell: Cell<Unknown>,
      readable: Cellable.Circuitless & Readable<T>
    ) {
      if (cell.command !== readable.command) throw new InvalidCommandError();
